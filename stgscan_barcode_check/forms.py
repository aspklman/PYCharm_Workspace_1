...
class BarcodeCheckForm(forms.Form):
    ...
    def clean_scan1(self):
        ...
        if num_words == 12:
            # 检查【工厂条码】是否错误
            outboxd = Outboxd.objects.filter(fact_no='0006',outbox_no=scan1)
            if not outboxd:
                winsound.Beep(300,600)
                raise forms.ValidationError('【工厂条码】错误，请检查！')
            # 检查是否有【入库扫描】
            stgscan_rec = Stgscan_rec.objects.filter(fact_no__icontains='0006',bar_no__icontains=scan1)
            if not stgscan_rec:
                winsound.Beep(300,600)
                raise forms.ValidationError('未做【入库扫描】，请检查！')
            # 检查【工厂条码】是否重复扫描
            stgscan_barcode_check = Stgscan_barcode_check.objects.filter(fact_no__icontains='0006', cont_no__icontains=scan1)
            if stgscan_barcode_check:
                winsound.Beep(300,600)
                raise forms.ValidationError('【工厂条码】重复扫描，请检查！')
    ...
    def clean_scan2(self):
        ...
        if num_words == 20:
            # 检查【客户条码】是否错误
            odrsscc_pp_xml = Odrsscc_pp_xml.objects.filter(marknumberfrom__icontains=scan2)
            if not odrsscc_pp_xml:
                winsound.Beep(300, 600)
                raise forms.ValidationError('【客户条码】错误，请检查！')
            # 检查【客户条码】是否重复扫描
            stgscan_barcode_check = Stgscan_barcode_check.objects.filter(fact_no__icontains='0006',bar_no__icontains=scan2)
            if stgscan_barcode_check:
                winsound.Beep(300, 600)
                raise forms.ValidationError('【客户条码】重复扫描，请检查！')
            # 获取【客户条码】所对应的sku_number,qty
            v_sis_sis_barcode_check_c = V_sis_sis_barcode_check_c.objects.get(fact_no='0006', ctn_no=scan2)
            if v_sis_sis_barcode_check_c:
                scan2 = v_sis_sis_barcode_check_c.sku_number
            # 检查【工厂条码】与【客户条码】是否对应
        else:
            winsound.Beep(300, 600)
            # wavefile = "voice\cuowu.wav"
    ...
    # def clean_ctn1(self):
    #     ctn1 = self.cleaned_data['ctn1']
    #     # 获取【工厂条码】所对应的sku_number,qty
    #     v_sis_sis_barcode_check_f = V_sis_sis_barcode_check_f.objects.filter(fact_no__icontains='0006', ctn_no__icontains=ctn1)
    #     return ctn1
    #
    # def clean_ctn2(self):
    #     ctn2 = self.cleaned_data['ctn2']
    #     # 获取【客户条码】所对应的sku_number,qty
    #     v_sis_sis_barcode_check_f = V_sis_sis_barcode_check_f.objects.filter(fact_no__icontains='0006', ctn_no__icontains=ctn2)
    #     return ctn2